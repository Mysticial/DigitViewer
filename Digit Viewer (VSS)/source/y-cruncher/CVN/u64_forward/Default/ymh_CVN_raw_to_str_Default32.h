/* ymi_CVN_raw_to_str_Default32.h
 * 
 * Author           : Alexander J. Yee
 * Date Created     : 04/04/2012
 * Last Modified    : 04/06/2012
 * 
 * 
 *      ymi             =   Micro-Intrinsic
 *      CVN             =   Format Conversion Unit
 *      raw_to_str      =   Raw -> String
 *      Default         =   Default Instruction Set
 * 
 */

#ifndef _ymh_CVN_raw_to_str_Default32_H
#define _ymh_CVN_raw_to_str_Default32_H

//  Dependencies
#include "../../ymb_CVN_headers.h"
#include "ymi_CVN_rawh_to_strh_Default.h"
#include "ymi_CVN_rawd_to_strd_Default.h"

void ymb_CVN_rawh_to_strh_f(char *T,ym_pL L){
    char *stop = T + L;

    //while (T < stop - 15){
    //    ymi_CVN_rawh_to_strh_u4_Default32(T);
    //    T += 16;
    //}
    while (T < stop - 7){
        ymi_CVN_rawh_to_strh_u3_Default32(T);
        T += 8;
    }
    while (T < stop - 3){
        ymi_CVN_rawh_to_strh_u2_Default32(T);
        T += 4;
    }
    while (T < stop){
        ymi_CVN_rawh_to_strh_u0_Default(T);
        T += 1;
    }
}

void ymb_CVN_rawd_to_strd_f(char *T,ym_pL L){
    char *stop = T + L;

    while (T < stop - 15){
        ymi_CVN_rawd_to_strd_u4_Default32(T);
        T += 16;
    }
    while (T < stop - 7){
        ymi_CVN_rawd_to_strd_u3_Default32(T);
        T += 8;
    }
    while (T < stop - 3){
        ymi_CVN_rawd_to_strd_u2_Default32(T);
        T += 4;
    }
    while (T < stop){
        ymi_CVN_rawd_to_strd_u0_Default(T);
        T += 1;
    }
}

#endif
